    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          ...
          <button onClick={() => { setSettingsOpen(p => !p); setInfoOpen(false) }} onMouseEnter={e=>{e.currentTarget.style.background='#0a0a0a';e.currentTarget.style.color='#fff'}} onMouseLeave={e=>{e.currentTarget.style.background='#f5f4f0';e.currentTarget.style.color='#888'}} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '4px 10px', background: '#f5f4f0', border: 'none', fontSize: '11px', color: '#888', cursor: 'pointer', transition: 'all 0.15s', flexShrink: 0 }}>
            ...
            <div key={video.id} ref={el => { videoCardRefs.current[video.id] = el }} style={{ display: 'flex', gap: '14px', padding: '14px', marginBottom: '8px', border: highlightId === video.id ? '2px solid #22c55e' : '1px solid var(--color-border-tertiary)', background: '#fff', alignItems: 'flex-start', transition: 'all 0.3s' }}>
              ...
              <div style={{ width: '200px', aspectRatio: (brief?.format || '9:16').replace(':', '/'), background: '#0a0a0a', flexShrink: 0, overflow: 'hidden', position: 'relative' }}>
                {isFailed ? (
                  ...
                  <>
                    <video src={video.final_url} controls controlsList="nodownload noplaybackrate" disablePictureInPicture preload="metadata" onPlay={e => { pauseOtherVideos(e.currentTarget); markViewed(video.id) }} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                  ...
                  <div style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" style={{ position: 'absolute', inset: '-5%', zIndex: 1, width: '110%', height: '110%', opacity: 0.6, mixBlendMode: 'screen' as const, animation: 'dinamoStatic 0.4s steps(4) infinite', pointerEvents: 'none' as const }} xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><filter id="dinamoNoise"><feTurbulence type="fractalNoise" baseFrequency="0.65" numOctaves="2" stitchTiles="stitch" /><feColorMatrix type="saturate" values="0" /></filter><rect width="100%" height="100%" filter="url(#dinamoNoise)" /></svg>
                    <div style={{ position: 'absolute', inset: 0, zIndex: 2, backgroundImage: 'radial-gradient(circle at 50% 45%, rgba(255,255,255,0.22) 0%, rgba(255,255,255,0.07) 30%, transparent 65%)', animation: 'dinamoPulseGlow 4.5s ease-in-out infinite' }} />
