            ...
            <div key={video.id} ref={el => { videoCardRefs.current[video.id] = el }} style={{ display: 'flex', gap: '14px', padding: '14px', marginBottom: '8px', border: highlightId === video.id ? '2px solid #22c55e' : '1px solid var(--color-border-tertiary)', background: '#fff', alignItems: 'flex-start', transition: 'all 0.3s' }}>
              ...
              <div style={{ width: '200px', aspectRatio: (brief?.format || '9:16').replace(':', '/'), background: '#0a0a0a', flexShrink: 0, overflow: 'hidden', position: 'relative' }}>
                {isFailed ? (
                  ...
                  <>
                    <video src={video.final_url} controls controlsList="nodownload noplaybackrate" disablePictureInPicture preload="metadata" onPlay={e => { pauseOtherVideos(e.currentTarget); markViewed(video.id) }} style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} />
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                  ...
                  {(() => { const snap = video.settings_snapshot || animSettings; return (<>
                    {snap.logo_enabled && <span style={{ fontSize: '9px', padding: '2px 6px', background: '#f5f4f0', color: '#888', letterSpacing: '0.5px', fontWeight: 600 }}>LOGO</span>}
                    {snap.cta_enabled && <span style={{ fontSize: '9px', padding: '2px 6px', background: '#f5f4f0', color: '#888', letterSpacing: '0.5px', fontWeight: 600 }}>CTA</span>}
                    {snap.packshot_enabled && <span style={{ fontSize: '9px', padding: '2px 6px', background: '#f5f4f0', color: '#888', letterSpacing: '0.5px', fontWeight: 600 }}>PACKSHOT</span>}
                  </>)})()}
                  {isPurchased && <span style={{ fontSize: '9px', color: '#1DB81D', fontWeight: '600' }}>&#10003; Satın Alındı</span>}
                  {isProcessing && <span style={{ fontSize: '9px', fontWeight: '500', display: 'inline-flex', alignItems: 'center', gap: '4px' }}><StatusDot color="#4ade80" size={6} pulse /><span style={{ color: '#0a0a0a' }}>Üretiliyor</span> <span style={{ color: '#6b6b66' }}>(~5 dakika)</span></span>}
