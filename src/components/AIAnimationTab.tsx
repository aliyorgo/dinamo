...
export default function AIAnimationTab({ briefId, brief, clientUser, onVideoCountChange }: Props) {
  ...
  async function handleGenerate(styleSlug: string) {
    ...
    try {
      ...
      if (data.animation_video_id) {
        ...
        setAnimationVideos(prev => [...prev, {
          id: data.animation_video_id, status: 'queued', style_slug: styleSlug, version: data.version,
          animation_styles: styleInfo ? { label: styleInfo.label, icon_path: styleInfo.icon_path } : null,
          created_at: new Date().toISOString(),
        }])
        setSelectedStyle(styleSlug)
        setHighlightId(data.animation_video_id)
  ...
  const selectedStyleVideos = selectedStyle ? animationVideos.filter(v => v.style_slug === selectedStyle) : []
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
          {styles.map(style => {
            ...
            const styleVideoCount = animationVideos.filter(v => v.style_slug === style.slug && v.status !== 'failed').length
            return (
              <div key={style.slug}
                onClick={() => { if (!generating) setSelectedStyle(style.slug) }}
                style={{ border: isSelected ? '2px solid #0a0a0a' : '1px solid #e5e4db', padding: '12px', textAlign: 'center', cursor: generating ? 'not-allowed' : 'pointer', background: isSelected ? '#fafaf7' : '#fff', transition: 'all 0.15s', position: 'relative' }}>
                ...
                {styleVideoCount > 0 && <span style={{ position: 'absolute', top: '6px', right: '6px', fontSize: '9px', fontWeight: '600', color: '#8b5cf6', background: 'rgba(139,92,246,0.08)', padding: '1px 5px' }}>{styleVideoCount}</span>}
              </div>
            )
          })}
        </div>
      ...
      {/* SEÇİLİ STİL İÇİN ÜRET + VİDEOLAR */}
      {selectedStyle && (
        <>
          {/* ÜRET BUTON */}
          <button
            ...
            style={{ width: '100%', padding: '14px', background: generating || credits < 1 ? '#ccc' : '#0a0a0a', color: '#fff', border: 'none', fontSize: '13px', fontWeight: '600', cursor: generating || credits < 1 ? 'default' : 'pointer', marginBottom: '20px', letterSpacing: '0.5px' }}>
            {generating ? 'ÜRETİLİYOR...' : totalCount === 0 ? 'ÜRET (ÜCRETSİZ)' : selectedStyleVideos.length === 0 ? 'ÜRET (1 KREDİ)' : 'YENİ VARYASYON ÜRET (1 KREDİ)'}
          </button>

          {/* VİDEO LİSTESİ — seçili stil */}
          {selectedStyleVideos.length > 0 && (
            <div>
              <div style={{ fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', color: '#888', fontWeight: '500', marginBottom: '12px' }}>
                {styles.find(s => s.slug === selectedStyle)?.label || selectedStyle} · {selectedStyleVideos.length} VIDEO
              </div>
              {selectedStyleVideos.map((video, idx) => {
                          ...
                          {isEditingFb ? (
                            <div style={{ display: 'flex', gap: '6px' }}>
                              ...
                              <button onClick={() => saveFeedback(video.id, versionLabel, video.style_slug)}
                                style={{ padding: '8px 14px', background: '#0a0a0a', color: '#fff', border: 'none', fontSize: '11px', fontWeight: '500', cursor: 'pointer', whiteSpace: 'nowrap' }}>Kaydet</button>
                            </div>
                          ) : lastFb ? (
