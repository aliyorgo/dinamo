...
export async function getAllCreditSettings(): Promise<Record<string, any>> {
  try {
    const { data } = await supabase.from('admin_settings').select('key, value').or('key.like.credit_%,key.eq.ai_express_global_enabled,key.eq.ugc_global_enabled,key.eq.animation_global_enabled')
