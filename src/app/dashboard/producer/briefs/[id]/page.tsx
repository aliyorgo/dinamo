...
export default function ProducerBriefDetail() {
  ...
  const inputStyle: React.CSSProperties = { width:'100%', boxSizing:'border-box', background:'#fff', border:'0.5px solid rgba(0,0,0,0.12)', borderRadius:'10px', padding:'9px 13px', fontSize:'14px', color:'#0a0a0a', fontFamily:'Inter,sans-serif', outline:'none' }
            ...
            <div style={{display:'flex',gap:'6px'}}>
              {['in_production','revision','approved','delivered'].map(s=>(
                <button key={s} onClick={()=>handleStatusChange(s)}
                  style={{padding:'5px 12px',borderRadius:'100px',border:'0.5px solid',borderColor:brief.status===s?'#111113':'rgba(0,0,0,0.15)',background:brief.status===s?'#111113':'#fff',color:brief.status===s?'#fff':'#555',fontSize:'10px',cursor:'pointer',fontFamily:'Inter,sans-serif'}}>
                  {statusLabel[s]}
                </button>
              ))}
            </div>
                ...
                <div style={{flex:'0.8',minWidth:0}}>
                  ...
                  <div style={{background:'#fff',border:'0.5px solid rgba(0,0,0,0.1)',borderRadius:'12px',padding:'16px',marginBottom:'12px'}}>
                    {assigned ? (
                      <div>
                        ...
                        <button onClick={()=>setShowAssignForm(!showAssignForm)} style={{fontSize:'12px',color:'#3b82f6',background:'none',border:'none',cursor:'pointer',fontFamily:'Inter,sans-serif',padding:0}}>
                    ...
                    {(showAssignForm||!assigned)&&(
                      <form onSubmit={handleForward} style={{marginTop:'10px'}}>
                        ...
                        <button type="submit" disabled={loading} style={{width:'100%',padding:'9px',background:'#111113',color:'#fff',border:'none',borderRadius:'8px',fontSize:'13px',cursor:'pointer',fontFamily:'Inter,sans-serif',fontWeight:'500'}}>
                        ...
                        {s.status==='pending'&&(
                          <div style={{padding:'0 12px 12px'}}>
                            <button onClick={()=>handleApprove(s.id)} disabled={loading}
                              style={{width:'100%',padding:'10px',background:'#22c55e',color:'#fff',border:'none',borderRadius:'8px',fontSize:'13px',cursor:'pointer',fontFamily:'Inter,sans-serif',fontWeight:'500',marginBottom:'8px'}}>
                  ...
                  <div style={{background:'#fff',border:'0.5px solid rgba(0,0,0,0.1)',borderRadius:'12px',padding:'16px'}}>
                      ...
                      <div key={q.id} style={{marginBottom:'6px',padding:'8px 10px',background:'#f5f4f0',borderRadius:'8px'}}>
                        ...
                        {q.answer ? (
                          ...
                          <div style={{display:'flex',gap:'6px',marginTop:'4px'}}>
                            <input value={answerText} onChange={e=>setAnswerText(e.target.value)} placeholder="Cevabı girin..." style={{flex:1,padding:'6px 10px',border:'0.5px solid rgba(0,0,0,0.12)',borderRadius:'6px',fontSize:'12px',color:'#0a0a0a',fontFamily:'Inter,sans-serif',outline:'none'}} />
                            <button onClick={()=>handleAnswerForClient(q.id)} style={{padding:'6px 12px',background:'#22c55e',color:'#fff',border:'none',borderRadius:'6px',fontSize:'10px',cursor:'pointer',fontFamily:'Inter,sans-serif'}}>Kaydet</button>
                            <button onClick={()=>setAnswerEditing(null)} style={{padding:'6px 10px',background:'#f5f4f0',color:'#555',border:'none',borderRadius:'6px',fontSize:'10px',cursor:'pointer',fontFamily:'Inter,sans-serif'}}>İptal</button>
                          </div>
                        ) : (
                    ...
                    <div style={{display:'flex',gap:'4px',flexWrap:'wrap',marginTop:'8px',marginBottom:'6px'}}>
                      {['Format tercihiniz?','Logo dosyası var mı?','Renk paleti?','Referans video?','Yayın tarihi?','Hedef platform?'].map(t=>(
                        <button key={t} onClick={()=>setQuestion(t)} type="button"
                          style={{padding:'4px 10px',borderRadius:'100px',border:'0.5px solid rgba(0,0,0,0.1)',background:'#f5f4f0',fontSize:'11px',color:'#555',cursor:'pointer',fontFamily:'Inter,sans-serif'}}>
                          {t}
                        </button>
                      ))}
                    </div>
                    <form onSubmit={handleQuestion} style={{display:'flex',gap:'6px'}}>
                      ...
                      <button type="submit" style={{padding:'9px 14px',background:'#111113',color:'#fff',border:'none',borderRadius:'8px',fontSize:'12px',cursor:'pointer',fontFamily:'Inter,sans-serif'}}>Gönder</button>
                    </form>
                  </div>
                </div>
              </div>
