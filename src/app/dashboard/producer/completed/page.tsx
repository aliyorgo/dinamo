    ...
    <div style={{display:'flex',minHeight:'100vh',fontFamily:'var(--font-dm-sans),sans-serif',background:'#f7f6f2'}}>
      <div style={{width:'240px',background:'#0A0A0A',padding:'32px 0',display:'flex',flexDirection:'column',flexShrink:0}}>
        ...
        <nav style={{flex:1,padding:'24px 0'}}>
          {NAV.map(item=>(
            <a key={item.href} href={item.href} style={{display:'block',padding:'10px 24px',fontSize:'11px',color:item.href==='/dashboard/producer/completed'?'#fff':'#888',textDecoration:'none',letterSpacing:'1px',fontFamily:'monospace'}}>{item.label}</a>
          ))}
        </nav>
      ...
      <div style={{flex:1,padding:'48px'}}>
        <h1 style={{fontSize:'28px',fontWeight:'300',letterSpacing:'-1px',margin:'0 0 40px',color:'#0a0a0a'}}>Tamamlananlar</h1>
        <div style={{background:'#fff',border:'1px solid #e8e7e3',borderRadius:'12px',overflow:'hidden'}}>
          {briefs.length === 0 ? (
            <div style={{padding:'48px',textAlign:'center',color:'rgba(255,255,255,0.4)',fontSize:'14px'}}>Tamamlanan proje yok.</div>
          ) : (
                ...
                {briefs.map((brief,i)=>(
                  <tr key={brief.id} style={{borderBottom:i<briefs.length-1?'1px solid #f0f0ee':'none',cursor:'pointer'}}
                    ...
                    <td style={{padding:'14px 20px'}}>
                      <span style={{fontSize:'11px',padding:'3px 10px',borderRadius:'100px',background:'#e8f7e8',color:'#1db81d',fontFamily:'monospace'}}>
                        Teslim Edildi
                      </span>
                    </td>
