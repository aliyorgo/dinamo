  ...
  useEffect(() => {
    async function load() {
      ...
      if (cu) {
        ...
        // Fallback: client_users eşleşmedi, tüm delivered brief'leri çek
        console.log('[Reports] client_users NULL — fallback: fetching all delivered briefs')
        const { data: b } = await supabase.from('briefs').select('*').eq('status', 'delivered').order('created_at', { ascending: false })
        console.log('[Reports] Fallback briefs:', b?.length)
        setBriefs(b || [])
      }
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Inter',system-ui,sans-serif", background: '#f5f4f0' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '14px 28px', background: '#fff', borderBottom: '0.5px solid rgba(0,0,0,0.08)', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={handlePdf} style={{ background: '#111113', color: '#fff', border: 'none', borderRadius: '8px', padding: '8px 18px', fontSize: '12px', fontWeight: '500', cursor: 'pointer', fontFamily: 'Inter,sans-serif', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.1)', borderRadius: '12px', padding: '16px 20px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                    style={{ padding: '7px 12px', borderRadius: '8px', border: '1px solid rgba(0,0,0,0.12)', fontSize: '13px', fontFamily: 'Inter,sans-serif', color: '#0a0a0a', outline: 'none' }} />
                ...
                {[
                  ...
                  <button key={p.label} onClick={p.fn}
                    style={{ padding: '6px 14px', borderRadius: '100px', border: '1px solid rgba(0,0,0,0.1)', background: '#fff', fontSize: '11px', color: '#555', cursor: 'pointer', fontFamily: 'Inter,sans-serif', fontWeight: '500' }}>
                    {p.label}
                  </button>
                ))}
                {(dateFrom || dateTo) && (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '950px' }}>
                      ...
                      <tbody>
                        {filteredBriefs.map(b => {
                          ...
                          const inputStyle: React.CSSProperties = { padding: '4px 8px', borderRadius: '6px', border: '1px solid rgba(34,197,94,0.5)', fontSize: '13px', fontFamily: 'Inter,sans-serif', outline: 'none', background: '#fff' }
                                ...
                                {isDelivered && b.public_link ? (
                                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                                    ...
                                    <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/video/${b.id}`); setCopiedLinkId(b.id); setTimeout(() => setCopiedLinkId(null), 1500) }}
                                      style={{ fontSize: '10px', padding: '3px 8px', borderRadius: '6px', border: '0.5px solid rgba(0,0,0,0.12)', background: '#fff', color: copiedLinkId === b.id ? '#22c55e' : '#888', cursor: 'pointer', fontFamily: 'Inter,sans-serif', whiteSpace: 'nowrap' }}>
                                      {copiedLinkId === b.id ? 'Kopyalandı ✓' : 'Kopyala'}
                                    </button>
                                  </span>
                                ) : isDelivered ? (
                                  <button onClick={() => generatePublicLink(b.id)} disabled={generatingLinkId === b.id}
                                    style={{ background: 'none', border: '1px solid rgba(0,0,0,0.12)', borderRadius: '6px', padding: '4px 10px', fontSize: '11px', color: generatingLinkId === b.id ? '#bbb' : '#555', cursor: 'pointer', fontFamily: 'Inter,sans-serif', whiteSpace: 'nowrap' }}>
