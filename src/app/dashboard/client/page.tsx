        ...
        <div style={{display:'grid',gap:'12px'}}>
          {briefs.length === 0 ? (
            <div style={{background:'#fff',border:'1px solid #e8e7e3',borderRadius:'12px',padding:'48px',textAlign:'center'}}>
              ...
              <a href="/dashboard/client/brief/new" style={{padding:'12px 24px',background:'#0a0a0a',color:'#fff',borderRadius:'100px',fontSize:'14px',textDecoration:'none',fontWeight:'500'}}>İlk Briefinizi Gönderin</a>
            </div>
          ) : briefs.map(brief => (
            <div key={brief.id} onClick={() => router.push(`/dashboard/client/briefs/${brief.id}`)}
              ...
              <div>
                <div style={{fontSize:'15px',fontWeight:'500',marginBottom:'4px'}}>{brief.campaign_name}</div>
                <div style={{fontSize:'13px',color:'#888'}}>{brief.video_type} · {new Date(brief.created_at).toLocaleDateString('tr-TR')}</div>
              </div>
