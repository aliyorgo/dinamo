          ...
          {savedBriefId && (
            <div style={{marginBottom:'32px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'20px'}}>
                ...
                <span style={{fontSize:'10px',padding:'2px 6px',background:'#0a4b1a',color:'#4ade80',borderRadius:'4px',letterSpacing:'0.05em',fontWeight:600}}>BETA</span>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(4, 1fr)',gap:'16px'}}>
                {[
                  {key:'express',label:'EXPRESS',sub:'~5 dakika',desc:'Hizli AI video, yonlendirmenize uygun uretilir.',video:'/express_v_tn.mp4',tab:'express'},
                  {key:'ugc',label:'PERSONA',sub:'~3 dakika',desc:'AI influencer\'lar kampanyayi anlatsin.',video:'/ugc_v_tn.mp4',tab:'ugc'},
                  {key:'animation',label:'ANIMATION',sub:'~5 dakika',desc:'Animasyon uretin. Marka maskotunuzu kullanin.',video:'https://liegyfgignwepqgswxhg.supabase.co/storage/v1/object/public/videos/marketing/ai-animation/a_main_1.mp4',tab:'animation'},
                  ...
                  <a key={item.key} href={`/dashboard/client/briefs/${savedBriefId}?tab=${item.tab}`} style={{display:'flex',flexDirection:'column',gap:'12px',textDecoration:'none'}}>
                    <div onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.transform='scale(1.03)';const wo=e.currentTarget.querySelector('[data-white-ov]') as HTMLElement;if(wo)wo.style.opacity='0';const rb=e.currentTarget.querySelector('[data-ribbon]') as HTMLElement;if(rb)rb.style.opacity='1'}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.transform='scale(1)';const wo=e.currentTarget.querySelector('[data-white-ov]') as HTMLElement;if(wo)wo.style.opacity='1';const rb=e.currentTarget.querySelector('[data-ribbon]') as HTMLElement;if(rb)rb.style.opacity='0'}} style={{position:'relative',aspectRatio:'9/16',borderRadius:'8px',overflow:'hidden',transition:'transform 0.2s ease'}}>
                      ...
                      <div data-ribbon="" style={{position:'absolute',top:28,right:-55,width:200,transform:'rotate(30deg)',background:'#1db81d',color:'#0a0a0a',textAlign:'center',padding:'8px 0',fontSize:11,fontWeight:700,letterSpacing:'0.2em',opacity:0,transition:'opacity 0.2s ease',pointerEvents:'none',boxShadow:'0 2px 8px rgba(0,0,0,0.25)'}}>HEMEN URET</div>
                    </div>
