          ...
          {!brief ? <div style={{color:'#888',fontSize:'14px'}}>Yükleniyor...</div> : (
            <>
              ...
              <div style={{display:'flex',flexWrap:'nowrap',alignItems:'center',marginBottom:'12px',gap:'8px'}}>
                ...
                <span style={{fontSize:'9px',padding:'2px 6px',background:'#1DB81D',color:'#fff',fontWeight:'600',borderRadius:'2px',letterSpacing:'0.5px'}}>BETA</span>
                      ...
                      <div key={child.id} id={`ai-child-${child.id}`} style={{display:'flex',gap:'14px',padding:'14px',marginBottom:'8px',border:'1px solid var(--color-border-tertiary)',background:'#fff',alignItems:'flex-start',transition:'background 0.15s'}}
                        ...
                        <div style={{flex:1,minWidth:0,paddingTop:'4px'}}>
                          <div style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'4px'}}>
                            ...
                            {(child.ai_express_settings_snapshot?.kling_prompt || child.ai_express_settings_snapshot?.seedance_prompt) && <span onClick={()=>setDebugPrompt({director:child.ai_express_settings_snapshot.director||'',prompt:child.ai_express_settings_snapshot.kling_prompt || child.ai_express_settings_snapshot.seedance_prompt})} style={{marginLeft:'6px',fontSize:'9px',color:'#888',cursor:'pointer',textDecoration:'underline'}}>Prompt</span>}
                          </div>
              ...
              {/* ═══ PREMIUM TVC SECTION — disabled, Premium V2 is now default engine ═══ */}
                      ...
                      <div key={child.id} id={`trend-child-${child.id}`} style={{display:'flex',gap:'14px',padding:'14px',marginBottom:'8px',border:'1px solid var(--color-border-tertiary)',background:'#fff',alignItems:'flex-start',transition:'background 0.15s'}}
                        ...
                        <div style={{flex:1,minWidth:0,paddingTop:'4px'}}>
                          <div style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'4px'}}>
                            ...
                            <span style={{marginLeft:'6px',fontSize:'9px',padding:'2px 6px',background:'#e8e1ff',color:'#5d4ec3',borderRadius:'3px',letterSpacing:'0.5px',fontWeight:500,textTransform:'uppercase'}}>{child.express_engine==='trend_oops'?'Ooops':child.express_engine==='trend_dans'?'Dans':child.express_engine==='trend_gokten'?'Gökten Gelen':'Bana Bak'}</span>
                          </div>
                          <div style={{fontSize:'11px',color:'#888',marginBottom:'10px'}}>{new Date(child.created_at).toLocaleDateString('tr-TR',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})}{child.completed_at && <><span style={{margin:'0 8px',color:'#ccc'}}>|</span><span style={{color:'#aaa'}}>{formatDuration(child.created_at, child.completed_at)}</span></>}</div>
                          {hasVideo && !isFailed && (
                  ...
                  <div style={{marginBottom:'5px'}}>
                    <div style={{display:'flex',flexWrap:'wrap',gap:'5px'}}>
                      {[
                        ...
                        { key: 'dansdansdans' as const, title: 'Dans', desc: 'Genç ve eğlenceli, stilize bir UGC video formatı.', credit: 'Yönetmen: Ege Tül · Müzik: DFX', video: '/videos/dansdansdans.mp4' },
                      ].map(fmt => {
                        ...
                        return (
                          <div key={fmt.key} onClick={() => setTrendFormat(fmt.key)} onMouseEnter={e => { const layer = e.currentTarget.querySelector('[data-darken]') as HTMLElement; if (layer) layer.style.opacity = '0' }} onMouseLeave={e => { const layer = e.currentTarget.querySelector('[data-darken]') as HTMLElement; if (layer) layer.style.opacity = '1' }} style={{width:'calc(50% - 2.5px)',position:'relative',cursor:'pointer',overflow:'hidden',transition:'all 0.2s'}}>
                            ...
                            <div style={{position:'absolute',inset:0,zIndex:2,display:'flex',flexDirection:'column',justifyContent:'flex-start',padding:'4px 12px'}}>
                              ...
                              <div style={{color:'rgba(255,255,255,0.85)',fontSize:'13px',lineHeight:1.3,textShadow:'0 1px 3px rgba(0,0,0,0.6)'}}>{fmt.desc}</div>
                              <div style={{color:'rgba(255,255,255,0.65)',fontSize:'13px',lineHeight:1.3,fontStyle:'italic',textShadow:'0 1px 3px rgba(0,0,0,0.6)'}}>{fmt.credit}</div>
                            </div>
