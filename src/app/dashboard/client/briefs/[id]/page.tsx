                    ...
                    {aiGenerating ? (
                      ...
                      <div>
                        <button onClick={()=>brief.product_image_url?setShowAiGenerate(true):handleStudioGenerate('character')} disabled={(clientUser?.allocated_credits||0)<1}
                          ...
                          {(() => { const cc = aiChildren.filter(c => c.ai_video_status === 'completed' || c.status === 'delivered').length; return cc < 3 ? `ÜRET · ${cc + 1}/3 ÜCRETSİZ` : `Yeni Versiyon Üret · ${creditSettings?.credit_ai_express_generate || 1} KREDİ` })()}
                        </button>
                        <div style={{fontSize:'13px',color:'#999',textAlign:'center',marginTop:'6px'}}>{(() => { const cc = aiChildren.filter(c => c.ai_video_status === 'completed' || c.status === 'delivered').length; return cc >= 3 ? `~5 dakika · ${creditSettings?.credit_ai_express_generate || 1} kredi` : `~5 dakika · ${3 - cc} ücretsiz kaldı` })()}</div>
                      </div>
                    )}
                  </div>}
                </div>
