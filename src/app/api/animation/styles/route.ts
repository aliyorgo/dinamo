...
export async function GET(req: NextRequest) {
  ...
  if (clientId) {
    ...
    const [{ data: assigned }, { data: mascotStyles }, { data: client }] = await Promise.all([
      ...
      supabase.from('clients').select('mascot_enabled, mascot_image_url, mascot_only_icon_url, mascot_hybrid_icon_url, mascot_name').eq('id', clientId).single(),
    ])
    ...
    return NextResponse.json({ styles: allStyles, hasMascot, mascotIcons: { mascot_only: client?.mascot_only_icon_url || null, mascot_hybrid: client?.mascot_hybrid_icon_url || null }, mascotName: client?.mascot_name || null })
  }
