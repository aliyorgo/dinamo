...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { ugc_scripts } = await req.json()
  if (!ugc_scripts || typeof ugc_scripts !== 'object') {
    return NextResponse.json({ error: 'ugc_scripts object required' }, { status: 400 })
  }

  const { error } = await supabase.from('briefs').update({ ugc_scripts }).eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  return NextResponse.json({ success: true, ugc_scripts })
}
