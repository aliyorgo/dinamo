...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { animation_voiceovers, last_animation_style } = await req.json()

  const updates: any = {}
  if (animation_voiceovers && typeof animation_voiceovers === 'object') updates.animation_voiceovers = animation_voiceovers
  if (last_animation_style) updates.last_animation_style = last_animation_style
  if (Object.keys(updates).length === 0) return NextResponse.json({ error: 'No updates' }, { status: 400 })

  const { error } = await supabase.from('briefs').update(updates).eq('id', id)
